<template>
  <form class="
      ...
      bg-white
      shadow
      overflow-hidden" @submit.prevent="submit">
    ...
    <div class="pb-4">
      ...
      <transition name="show-date-controllers" enter-active-class="transition ease-out duration-300" enter-from-class="opacity-0 scale-90" enter-to-class="opacity-100 scale-100"
        leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-90">
        <!-- Date -->
        <div v-show="form.setDate" class="mb-2">
          ...
          <div class="grid grid-cols-12 items-center">
            ...
            <JetInput type="date" id="transactionDate" class="col-span-9 w-full text-sm" v-model="form.date" :max="maxDate" />
          ...
          <!-- Checkbox for time -->
          <div class="flex items-center mb-2">
            <JetCheckbox name="setDate" id="setTime" class="mr-2" v-model="form.setTime" />
            <label for="setTime" class="text-sm text-gray-800">Seleccionar hora</label>
          </div>

          <transition name="show-date-controllers" enter-active-class="transition ease-out duration-200" enter-from-class="opacity-0 scale-90" enter-to-class="opacity-100 scale-100"
            leave-active-class="transition ease-in duration-200" leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-90">
            <div v-show="form.setTime" class="grid grid-cols-12 items-center">
              <label for="transactionTime" class="col-span-3 text-sm text-gray-800">Hora:
              </label>
              <JetInput type="time" name="transactionTime" class="col-span-9 w-full text-sm" id="transactionTime" v-model="form.time" />
      ...
      <!-- Description -->
      <div class="mb-2">
        <label for="description" class="block mb-2 text-basetext-gray-800 text-center">Descripcion</label>
        <textarea name="description" id="description" rows="2" class="
            block
            w-full
            p-2
            border border-gray-300
          ...
          " placeholder="Escribe la descripción." v-model="form.description"></textarea>
        <jet-input-error :message="form.errors.description" class="mt-2" />
      </div>
      <!-- Importe -->
      <div class="mb-2">
        <label for="amount" class="inline-block mb-1 text-sm text-gray-800">Importe</label>
        <CurrencyInput name="amount" id="amount" type="text" class="w-full text-sm text-gray-800 text-right" v-model="form.amount" placeholder="Ingresa el importe" autocomplete="off" />
        <jet-input-error :message="form.errors.amount" class="col-span-12 mt-2" />
      </div>
      <!-- Tipo de transacción -->
      <div class="flex justify-around">
        <div class="flex items-center">
          <input type="radio" name="income" id="transactionIncome" v-model="form.type" value="income" class="
              mr-2
              border-gray-300
              focus:border-indigo-300
              focus:ring
        ...
        <div class="flex items-center">
          <input type="radio" name="expense" id="transactionExpense" v-model="form.type" value="expense" class="
              mr-2
              border-gray-300
              focus:border-indigo-300
              focus:ring
    ...
    <footer class="
      ...
      ">
      <JetButton>
        <svg class="animate-spin -ml-1 mr-3 h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" v-show="processing">
          <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" stroke-width="4"></circle>
          <path class="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path>
        </svg>
        {{ buttonMessage }}
      </JetButton>
      <JetDangerButton type="button" @click="hiddenForm">Cancelar</JetDangerButton>
    </footer>
  </form>
</template>
<script>
